...
const Footer = () => {
  ...
  const socialLinks = [
    ...
    { name: 'instagram', icon: Instagram, href: 'https://www.instagram.com/helpfy.app?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw==' },
    { name: 'linkedin', icon: Linkedin, href: '#' },
  ];

  return (
    <footer ref={ref} className="bg-navy text-white pt-16 pb-8 relative overflow-hidden">
      {/* Parallax Background */}
      <motion.div 
        style={{ y: backgroundY }}
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
          <div className="col-span-1 md:col-span-2">
            ...
            <p className="text-white/60 max-w-sm mb-6">
              Conectando você aos melhores profissionais com segurança, inteligência e conveniência.
            </p>
          ...
          <div>
            ...
            <ul className="space-y-2 text-white/60">
              {['Sobre', 'Planos', 'Blog', 'Carreiras', 'Contato'].map((link, index) => (
                <motion.li 
                  key={link}
                ...
                >
                  <a href="#" className="hover:text-coral transition-colors">{link}</a>
                </motion.li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-white/60">
              {['Termos de Uso', 'Privacidade', 'Cookies', 'LGPD'].map((link, index) => (
                <motion.li 
                  key={link}
                  initial={{ opacity: 0, x: -10 }}
                  whileInView={{ opacity: 1, x: 0 }}
        ...
        >
          <p>© {new Date().getFullYear()} Helpfy. Todos os direitos reservados.</p>
          <p className="mt-2">
