...
const PlansSection = () => {
  const freePlan = {
    ...
    price: "R$0",
    period: "/mês",
    description: "Para começar a oferecer seus serviços",
    icon: null,
    features: [
      { text: "Perfil básico com até 3 fotos", included: true },
      { text: "Até 3 leads por mês", included: true },
      { text: "Chat básico com clientes", included: true },
      { text: "Avaliações públicas", included: true },
      { text: "Comissão: 20% sobre serviços", included: true },
    ],
    limitations: [
      "Posição básica nas buscas",
  ...
  */
  ...
  return (
    <section className="section-padding bg-gradient-to-b from-white to-cream/30" id="plans">
      <div className="container mx-auto px-4">
        <ScrollAnimation className="text-center mb-16">
          <span className="inline-block text-coral font-semibold text-sm uppercase tracking-widest mb-4">Plano para Prestadores</span>
          <h2 className="section-heading text-navy">Comece Gratuitamente</h2>
          <p className="text-lg text-navy/60 max-w-2xl mx-auto">Ofereça seus serviços sem custo inicial. Crie seu perfil e comece a receber clientes hoje mesmo.</p>
        </ScrollAnimation>
          ...
          <StaggerItem>
            ...
            >
              ...
              <div className="text-center mb-8">
                <h3 className="text-2xl font-bold text-navy mb-2">{freePlan.name}</h3>
                <div className="mb-3">
                  <span className="text-5xl font-bold text-coral">{freePlan.price}</span>
                  <span className="text-navy/60 text-lg">{freePlan.period}</span>
                </div>
                <p className="text-navy/70">{freePlan.description}</p>
              </div>
              ...
              <ul className="space-y-3 mb-8">
                ...
                {freePlan.limitations.map((limitation, idx) => (
                  <motion.li 
                    ...
                    whileInView={{ opacity: 1, x: 0 }}
                    viewport={{ once: true }}
                    transition={{ delay: 0.2 + (freePlan.features.length + idx) * 0.05 }}
                    className="flex items-start gap-3"
                  >
